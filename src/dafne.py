...
def distanza(x1, y1, x2, y2): 
	d12 = math.sqrt((x1-x2)**2 + (y1-y2)**2) #Euclidean Distance
	return d12
...
def ricomponi(frammento , immagine_intera, output):
	...
	img1 = cv2.imread(img1_filename, 0) # fragments
	img2 = cv2.imread(whole_image, 0) # whole image
	...
	knn_matches = matcher.knnMatch(sift_des1, sift_des2, 2)

	ratio_thresh = 0.7
	good_matches = []

	idx1 = []	# keypoints indexes in the fragment
	idx2 = []	# keypoints indexes in the big image
	...
	for punto1 in good_matches:
		...
		for punto2 in good_matches:
			...
			dist_mom = distanza (sift_kp2[origine].pt[0] ,sift_kp2[origine].pt[1], sift_kp2[destino].pt[0], sift_kp2[destino].pt[1])
